...
class ExcelProcessorGUI:
    ...
    def choose_file(self, event):
        ...
        if file_path:
            ...
            self.file_path_var.set("点击此处选择文件")
    ...
    def process_inner(self, sheet):
        try:
            ...
            for i, row in enumerate(rows):
                if (i % 10000 == 0) or (i == sheet.max_row - 1):
                    with self.lock:
                        print(f"正在处理第 {i+1} 行，共 {sheet.max_row} 行，剩余 {sheet.max_row-i} 行")
                        self.process_iter = i
            ...
            for i, data in enumerate(data_dict.values()):
                if (i % 10 == 0) or (i == len(data_dict) - 1):
                    with self.lock:
